...
public class ScalaScriptProvider extends GhidraScriptProvider {
	 ...
	 * @return the bundle
	 */
	public GhidraScalaSourceBundle getBundleForSource(ResourceFile sourceFile) {
		ResourceFile sourceDir = GhidraScriptUtil.findSourceDirectoryContaining(sourceFile);
		if (sourceDir == null) {
			return null;
		}
		return (GhidraScalaSourceBundle) bundleHost.getExistingGhidraBundle(sourceDir);
	}
	...
	/**
	 * Activate and build the {@link GhidraScalaSourceBundle} containing {@code sourceFile} 
	...
	public Class<?> loadClass(ResourceFile sourceFile, PrintWriter writer) throws Exception {
		GhidraScalaSourceBundle bundle = getBundleForSource(sourceFile);
		if (bundle == null) {
